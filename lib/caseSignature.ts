...
// Формат шифру справи.
//
// Регекси й пороги виведені з аналізу наявних шифрів робочої бази.
//
//   SANITY — базова перевірка ЦІЛОГО шифру: є хоча б одна літера і одна цифра,
//            лише дозволені символи, довжина 3–90. Відкидає реальне сміття:
//            зворотний слеш, «?», рядки без цифр, завеликий текст.
//            Застосовується до case_signature (іноземний архів — вводиться
//            руками), до КОЖНОЇ додаткової сигнатури і до зібраного
//            українського шифру.
//
//   PART   — дозволені символи ОКРЕМОЇ складової українського шифру:
//            архів, фонд, опис, справа.
//
// Чому складові перевіряються поокремо, а не маскою по зібраному шифру.
//
// Раніше тут була структурна маска «АРХІВ Ф-О-С» на весь рядок. Вона виходила
// з того, що дефіс — роздільник, і всередині складової його бути не може. У
// реальних шифрах це не так: дефіс і крапка є легальним ВМІСТОМ складової
// («КМФ-15» як фонд, «2 дод.» як опис, «39/І-1» чи «377 Ч. 1.» як справа), а
// опис іноді записують просто як «-». Через це маска відкидала справжні шифри
// — на робочій базі 212 записів у 29 варіантах, серед них 108 записів
// «ЛННБ ім. Стефаника 91---39/І-1».
//
// Зібраний шифр із такими складовими розібрати назад однозначно неможливо, тож
// структурної перевірки по ньому й не має бути: формат гарантує сам
// buildCaseSignature, а зміст — перевірка кожної складової окремо.
// ---------------------------------------------------------------------------
...
/**
 * Складова українського шифру: літери будь-якої абетки (зокрема кирилиця),
 * цифри, крапка, дефіс і тире, а також пробіл, кома, дужки, скісна риска й
 * апостроф — усе це трапляється в наявних шифрах. Вимоги мати цифру НЕМАЄ:
 * опис «-» означає «без опису» і теж має проходити.
 */
export const CASE_SIGNATURE_PART = /^[\p{L}\p{N} .,()\/\-–‑—'’ʼ]+$/u;
...
export const SIGNATURE_PART_HINT =
  'Дозволені літери (зокрема кириличні), цифри, крапка, дефіс і тире, ' +
  'а також пробіл, кома, дужки та скісна риска. Приклади: «КМФ-15», «2 дод.», «377 Ч. 1.», «-».';

// Підписи складових для повідомлень про помилку. Локальні, а не з recordFields:
// той модуль сам імпортує звідси, і взаємний імпорт нам тут не потрібен.
const ARCHIVE_PART_LABELS: Record<string, string> = {
  archive: 'Архів',
  fonds: 'Фонд',
  series: 'Опис',
  record: 'Справа',
...
/**
 * Формат ОДНІЄЇ складової українського шифру.
 * Порожнє значення вважається валідним — обов'язковість перевіряється окремо.
 */
export function validateArchivePart(value: any, label: string): string | null {
  const part = text(value);
  if (part === '') return null;
  if (!CASE_SIGNATURE_PART.test(part)) {
    return `${label} «${part}» має недопустимий формат. ${SIGNATURE_PART_HINT}`;
  }
  return null;
}
...
export function validateCaseSignature(record: any): string | null {
  ...
  if (isUkrainian) {
    ...
    // Кожну складову перевіряємо окремо (див. коментар до CASE_SIGNATURE_PART),
    // а зібраний шифр — лише на базову осмисленість.
    for (const field of ARCHIVE_PART_FIELDS) {
      const partError = validateArchivePart(record?.[field], ARCHIVE_PART_LABELS[field]);
      if (partError) return partError;
    }

    const built = buildCaseSignature(record);
    if (!CASE_SIGNATURE_SANITY.test(built)) {
