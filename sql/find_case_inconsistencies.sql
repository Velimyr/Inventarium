...
--
-- Групуємо за ТОЧНИМ значенням шифру, без жодної нормалізації: так група
...
as $$
  ...
  select
    ...
    jsonb_strip_nulls(jsonb_build_object(
      'archive',                   case when g.c_archive > 1       then to_jsonb(g.v_archive) end,
      ...
      'additional_case_signature', case when g.c_add_signature > 1 then to_jsonb(g.v_add_signature) end
    ))
  from grouped g
  where g.c_archive > 1
     or g.c_fonds > 1
     ...
     or g.c_scans_url > 1
     or g.c_add_signature > 1
  -- спершу розбіжності в архівних координатах: це майже завжди помилка введення
  order by
    case when g.c_archive > 1 or g.c_fonds > 1 or g.c_series > 1 or g.c_record > 1 then 0 else 1 end,
    g.cnt desc,
    g.sig;
$$;
