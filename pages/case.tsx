...
// Імпортуємо Leaflet тільки на клієнті
let L: any;
...
// Динамічний імпорт GeocoderControl
...
// Створюємо іконки тільки на клієнті
const getIcons = () => {
    if (typeof window === 'undefined' || !L) return { blueIcon: null, redIcon: null };
...
export default function CasePage() {
    ...
    useEffect(() => {
        ...
        const fetchRecords = async () => {
            ...
            // Отримуємо записи з таблиці records
            ...
            if (error) {
                ...
                // Зберігаємо інформацію про справу з першого запису
    ...
    const getMapCenter = (): [number, number] => {
        ...
        if (validRecords.length === 0) return [48.3794, 31.1656]; // Центр України за замовчуванням
    ...
    // Отримуємо іконки
    const { blueIcon, redIcon } = getIcons();
                                        ...
                                        {records.map((record) => {
                                            ...
                                            return (
                                                <Marker 
                                                    key={record.id} 
                                                    position={position} 
                                                    icon={isRegion ? (redIcon || undefined) : (blueIcon || undefined)}
                                                >
                                                    <Popup>
                                                        <div>
