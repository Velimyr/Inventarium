// Тип документа, описаного записом: інвентар, люстрація, фасія, урбар чи дефтер.
//
...
// форма, адмінка й SQL-міграції наявних даних (sql/2026-08-04_inventory_type.sql,
// sql/2026-09-13_inventory_type_defter.sql) говорили про одне й те саме.

export const INVENTORY_TYPES = ['Інвентар', 'Люстрація', 'Фасія', 'Урбар', 'Дефтер'] as const;
...
const RULES: InventoryTypeRule[] = [
  {
    type: 'Дефтер',
    // Османські дефтери зберігаються в Національній бібліотеці святих Кирила і
    // Мефодія (Софія). Шифр має містити обидва слова, порядок не важливий.
    matches: (record) => {
      const signature = text(record?.case_signature).toLowerCase();
      return signature.includes('кирила') && signature.includes('мефодія');
    },
  },
  {
    type: 'Фасія',
 ...
 * (Дефтер → Фасія → Урбар → Люстрація), інакше «Інвентар».
 */
 ...
 *                                       дефтер, але в шифрі немає «Кирила» й «Мефодія».
 *
